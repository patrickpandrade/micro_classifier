...
TfLiteStatus InitializeMicroFeatures() {
  ...
  //config.noise_reduction.smoothing_bits = 10; //DESATIVADO PORQUE ESTA REPETIDO
  config.filterbank.num_channels = kFeatureSliceSize;
  //config.filterbank.lower_band_limit = 125.0;
  //config.filterbank.upper_band_limit = 7500.0;
  config.filterbank.lower_band_limit = 0.0;
  config.filterbank.upper_band_limit = 7999;
  config.noise_reduction.smoothing_bits = 10; //ALTERADO DE 10 PARA 2
  config.noise_reduction.even_smoothing = 0.025;
  config.noise_reduction.odd_smoothing = 0.06;
  //config.noise_reduction.min_signal_remaining = 0.05;
  config.noise_reduction.min_signal_remaining = 0.8; //MODIFICADO DE 0.05 PARA 0.8
  //config.pcan_gain_control.enable_pcan = 1;
  config.pcan_gain_control.enable_pcan = 0; //DESABILITADO AQUI E DESATIVADO REDUCAO DE RUIDO NO FRONTEND.C
...
TfLiteStatus GenerateMicroFeatures(const int16_t* input, int input_size,
 ...
 for (size_t i = 0; i < frontend_output.size; ++i) {
    ...
    */
    ...
    int32_t value = ((frontend_output.values[i] * value_scale) / value_div);
	// ************* Fim da modificação********************
