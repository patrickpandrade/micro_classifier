...
void RespondToCommand(int32_t current_time, const char* found_command,
  ...
  if (is_new_command) {
    MicroPrintf("TF MICRO: Heard %s (%d) @%dms", found_command, score, current_time);
    ...
    if (found_command[0] == 'B') {
      ...
      digitalWrite(LEDB, LOW);  // Blue for 'Mediano'
    } else if (found_command[0] == 'R') {
      digitalWrite(LEDR, LOW);  // Red for 'Ruim'
    } else if (found_command[0] == 'A') {
      digitalWrite(LEDR, 0);  // Yellow for 'Ambiente'
      digitalWrite(LEDG, 0);
      digitalWrite(LEDB, 255);
    } else {
      digitalWrite(LEDR, 200);	// Gree for 'Silence'
      digitalWrite(LEDG, 200);
